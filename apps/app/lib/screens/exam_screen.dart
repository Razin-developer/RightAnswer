...
class ExamScreen extends StatefulWidget {
  ...
  const ExamScreen({super.key, this.initialExamId});
...
class _ExamScreenState extends State<ExamScreen> {
  ...
  bool _didConsumeInitialExam = false;
  ...
  Future<void> _load() async {
    ...
    final initialExamId = widget.initialExamId;
    if (_didConsumeInitialExam || initialExamId == null) {
      return;
    }

    _didConsumeInitialExam = true;
    final match = exams.where((exam) => exam.id == initialExamId).firstOrNull;
  ...
  Future<void> _startExam(Exam exam) async {
    ...
    if (exam.maxAttempts > 0 && attemptCount >= exam.maxAttempts) {
      ...
      AppFeedback.showToast(
        context,
        'Attempt limit reached (${exam.maxAttempts})',
      );
      return;
    }
  ...
  Future<void> _openEdit(Exam exam) async {
    ...
    await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (_) =>
            ExamCreateScreen(exam: exam, existingQuestions: questions),
      ),
    );
    _load();
  }
  ...
  Future<void> _shareExam(Exam exam) async {
    if (!AuthService.instance.isLoggedIn) {
      AppFeedback.showToast(context, 'Sign in to share exams');
      return;
    }
    ...
    try {
      final bytes = await ImportExportService.instance.exportExamToBytes(
        exam.id,
      );
      final result = await CloudSyncService.instance.uploadContentZip(
        bytes: bytes,
        metadata: {
          'type': exam.type == 'mcq' ? 'quiz' : 'exam',
          'name': exam.name,
        },
      );
      final url = result['url'] as String? ?? '';
      if (mounted) _showLinkDialog('Share Exam', url);
    } catch (e) {
...
class _ExamCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final attemptsLeft = exam.maxAttempts > 0
        ...
        : null;
    final limitReached =
        exam.maxAttempts > 0 && attemptCount >= exam.maxAttempts;
              ...
              children: [
                ...
                if (exam.timeLimit != null)
                  _StatItem(
                    ...
                    label: '${exam.timeLimit}m',
                    color: mutedColor,
                  ),
                _StatItem(
                  icon: Icons.star_outline_rounded,
                  label: '${exam.totalMarks.toStringAsFixed(0)} pts',
                  color: mutedColor,
                ),
                _StatItem(
