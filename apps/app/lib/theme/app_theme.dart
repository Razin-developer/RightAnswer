...
// ── Public helpers for consistent font access across the app ──────────────────
extension AppTypography on BuildContext {
  TextStyle get displaySerifLg => _playfair(size: 28, height: 1.2, letterSpacing: -0.3);
  TextStyle get displaySerifMd => _playfair(size: 22, height: 1.25, letterSpacing: -0.2);
  TextStyle get bodyInter => _inter(size: 14, height: 1.55);
  TextStyle get monoCode => _mono(size: 13);
}

/// Convenience class so any widget can access canonical font styles without
/// a BuildContext (e.g. MarkdownStyleSheet builder).
class AppFonts {
  AppFonts._();
