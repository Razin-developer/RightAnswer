...
class RichAnswerView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final parsed = RichAnswerPayload.tryParse(content);
    if (parsed == null) {
      return _MarkdownAnswer(content: content, isDark: isDark);
    }

    final widgets = <Widget>[];
    if (parsed.renderMarkdown.trim().isNotEmpty) {
      widgets.add(
        _MarkdownAnswer(content: parsed.renderMarkdown, isDark: isDark),
      );
    }
    ...
    if (parsed.sources.isNotEmpty) {
      ...
      widgets.add(_SourceList(sources: parsed.sources, isDark: isDark));
    }
  ...
  Widget? _buildBlock(BuildContext context, Map<String, dynamic> block) {
    ...
    return switch (type) {
      'markdown' => _MarkdownAnswer(
          content: block['content']?.toString() ?? '',
          isDark: isDark,
        ),
      'math' => _MarkdownAnswer(
          content: (block['display'] == false)
              ? r'$' '${block['latex'] ?? ''}' r'$'
              : '\$\$\n${block['latex'] ?? ''}\n\$\$',
          isDark: isDark,
        ),
      'table' => _TableBlock(block: block, isDark: isDark),
      'chart' => _ChartBlock(block: block, isDark: isDark),
      'geometry' => _GeometryBlock(block: block, isDark: isDark),
      'svg' => _SvgBlock(block: block, isDark: isDark),
      'image' => _ImageBlock(block: block, isDark: isDark),
      'code' => _MarkdownAnswer(
          content:
              '```${block['language'] ?? ''}\n${block['code'] ?? ''}\n```',
          isDark: isDark,
        ),
      'quote' => _MarkdownAnswer(
          content: '> ${block['content'] ?? block['text'] ?? ''}',
          isDark: isDark,
        ),
      'callout' => _CalloutBlock(block: block, isDark: isDark),
      'timeline' => _TimelineBlock(block: block, isDark: isDark),
      _ => null,
    };
  }
}

class RichAnswerPayload {
  final String renderMarkdown;
  final List<Map<String, dynamic>> blocks;
  final List<Map<String, dynamic>> sources;

  const RichAnswerPayload({
    required this.renderMarkdown,
    required this.blocks,
    required this.sources,
  });

  static RichAnswerPayload? tryParse(String raw) {
    ...
    try {
      ...
      final hasRichShape =
          ...
          decoded.containsKey('speechText') ||
          decoded.containsKey('blocks');
      if (!hasRichShape) return null;

      return RichAnswerPayload(
        renderMarkdown: decoded['renderMarkdown']?.toString() ?? '',
        blocks: _maps(decoded['blocks']),
        sources: _maps(decoded['sources']),
      );
    } catch (_) {
      return null;
    }
        ...
        child: chartType == 'line'
            ? LineChart(
                LineChartData(
                  ...
                  lineBarsData: series.map((values) {
...
class _GeometryBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (final point in _maps(block['points'])) {
    ...
    final objects = _maps(block['objects']);
...
class _GeometryPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final object in objects) {
      ...
      if (kind == 'polygon') {
      ...
      } else if (kind == 'line' || kind == 'sideLabel') {
        ...
        if (from != null && to != null) {
          ...
          if (label != null) _drawText(canvas, label, (tx(from) + tx(to)) / 2);
        }
      } else if (kind == 'angle' || kind == 'rightAngle') {
        final vertex = points[object['vertex']?.toString()];
        final from = points[object['from']?.toString() ?? object['point1']?.toString()];
        final to = points[object['to']?.toString() ?? object['point2']?.toString()];
        if (vertex != null && from != null && to != null) {
          ...
          if (label != null) {
            ...
            _drawText(canvas, label, v + Offset(math.cos(mid), math.sin(mid)) * 42);
          }
        }
      }
    }
      ...
      child: Column(
        ...
        children: sources.take(5).map((source) {
          final page = source['pageNumber'] == null
              ? ''
              : ' p. ${source['pageNumber']}';
          final title = source['title']?.toString() ?? source['kind']?.toString() ?? 'Source';
          final snippet = source['snippet']?.toString() ?? '';
          return Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: Text(
              '$title$page${snippet.isEmpty ? '' : ': $snippet'}',
              style: GoogleFonts.inter(
                fontSize: 12.5,
                height: 1.45,
                color: isDark ? Colors.white70 : Colors.black87,
