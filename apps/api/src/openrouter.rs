...
fn extract_rich_envelope(raw: &str) -> Option<RichEnvelope> {
    ...
    // Defense in depth against the model still emitting an "image" block
    // despite being told not to (see build_system_prompt) — it never has a
    // real image URL, only the retrieved sources do, so any block claiming
    // to be one is necessarily hallucinated/wrong and must never reach the
    // client.
    let blocks = object.get("blocks").cloned().map(|value| match value {
...
fn build_system_prompt(
    ...
    if rich || json_mode {
        ...
        lines.push("Use Markdown, LaTeX, tables, charts, geometry, SVG, code, or timeline blocks only when useful. A chart/graph block should appear whenever the answer describes data, trends, or comparisons the context provides numbers for. Never emit an \"image\" block yourself — you do not have real image URLs; any page image worth showing is attached automatically from the retrieved sources, not generated by you.".into());
        lines.push("The renderer understands Markdown and LaTeX only, never raw HTML — never write <sub>, <sup>, or any other HTML tag, they show up as literal text. For every exponent, subscript, chemical formula, or equation (in renderMarkdown or inside blocks), use LaTeX instead: inline like $x^2$ or $H_2O$, standalone equations wrapped in $$...$$.".into());
