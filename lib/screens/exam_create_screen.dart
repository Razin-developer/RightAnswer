...
class _ExamCreateScreenState extends State<ExamCreateScreen> {
  ...
  Exam _buildExam(String? existingId) {
    ...
    return Exam(
      ...
      name: _nameCtrl.text.trim().isEmpty ? 'Untitled Exam' : _nameCtrl.text.trim(),
      type: _type,
  ...
  Future<void> _generateWithAI() async {
    ...
    if (!AppConfig.hasOpenAiApiKey) {
      await AppFeedback.showErrorDialog(context,
          AppException.configuration('Missing OPENAI_API_KEY dart-define.'));
      return;
    }
    ...
    try {
      ...
      final questions = result.questions.map((q) => ExamQuestion(
  ...
  Future<void> _refineWithAI() async {
    ...
    try {
      ...
      setState(() { _questions = questions; _aiPromptCtrl.clear(); });
      await _saveAll();
    } on AppException catch (e) {
      if (mounted) AppFeedback.showErrorDialog(context, e);
    } catch (e) {
  ...
  void _addQuestionManually() {
    ...
    final q = ExamQuestion(
      ...
      question: '',
      options: _type == 'true_false' ? ['True', 'False']
          ...
          if (_questions.isNotEmpty || _savedExam != null)
            TextButton(
              ...
              child: Text('Save', style: GoogleFonts.inter(
                fontSize: 14, fontWeight: FontWeight.w600, color: _coral,
              )),
            ),
        ],
      ),
              ...
              children: [
                  ...
                  child: Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text('Exam Settings', style: GoogleFonts.inter(
                ...
                if (_settingsExpanded) ...[
                  ...
                  TextField(
                    ...
                    decoration: _inputDec('e.g. Biology Chapter 3 Quiz', isDark),
                  ),
                  const SizedBox(height: 14),
                  // Type
                  ...
                  if (_type == 'mcq' || _type == 'mixed') ...[
                    ...
                    const SizedBox(height: 6),
                    Row(children: [3, 4, 5].map((n) => Padding(
                      padding: const EdgeInsets.only(right: 8),
                      child: ChoiceChip(
                        label: Text('$n', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w500)),
                        selected: _mcqOptionCount == n,
                        onSelected: (_) => setState(() => _mcqOptionCount = n),
                        selectedColor: _coral.withValues(alpha: 0.15),
                        labelStyle: TextStyle(color: _mcqOptionCount == n ? _coral : mutedColor),
                        side: BorderSide(color: _mcqOptionCount == n ? _coral : borderColor),
                      ),
                    )).toList()),
                  ],
                  const SizedBox(height: 14),
                  ...
                  OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(
                      ...
                      side: BorderSide(color: _chapterIds.isEmpty ? borderColor : _coral),
                    ),
            ...
            child: Column(
              ...
              children: [
                Row(children: [
                  ...
                  const SizedBox(width: 8),
                  Text('Generate with AI', style: GoogleFonts.inter(
                    fontSize: 15, fontWeight: FontWeight.w600, color: textColor,
                  )),
                  const Spacer(),
                  if (_questions.isNotEmpty)
                    GestureDetector(
                      ...
                      child: Text(
                        ...
                        style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: _coral),
                      ),
                    ),
                ]),
                ...
                Row(children: [
                  Expanded(
                    child: FilledButton.icon(
                      onPressed: _isGenerating ? null : (_isEditing ? _refineWithAI : _generateWithAI),
                      style: FilledButton.styleFrom(
                        backgroundColor: _coral,
                        disabledBackgroundColor: _coral.withValues(alpha: 0.4),
                      ...
                      label: Text(
                        _isGenerating
                            ...
                            : _isEditing
                            ? 'Refine Questions'
                            : (_questions.isEmpty ? 'Generate Questions' : 'Regenerate'),
          ...
          Row(
            children: [
              ...
              TextButton.icon(
                ...
                label: Text('Add', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w500)),
              ),
            ],
          ),
  ...
  InputDecoration _inputDec(String hint, bool isDark) {
    ...
    return InputDecoration(
      hintText: hint,
      hintStyle: GoogleFonts.inter(fontSize: 14, color: isDark ? const Color(0xFF8E8B82) : const Color(0xFF8E8B82)),
      filled: true,
      fillColor: fill,
      ...
      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: _coral, width: 2)),
    );
  }
}
...
class _Label extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Text(
    ...
    style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600,
        color: textColor.withValues(alpha: 0.6), letterSpacing: 0.3),
  );
}
      ...
      children: _types.map((t) {
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: isSelected ? _coral.withValues(alpha: 0.12) : Colors.transparent,
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: isSelected ? _coral : const Color(0xFFE6DFD8)),
            ),
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Icon(t.$3, size: 13, color: isSelected ? _coral : const Color(0xFF8E8B82)),
...
class _DifficultyDropdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DropdownButtonFormField<String>(
      ...
      items: ['easy', 'medium', 'hard', 'mixed'].map((d) => DropdownMenuItem(
        value: d, child: Text(d[0].toUpperCase() + d.substring(1), style: GoogleFonts.inter(fontSize: 14, color: textColor)),
      )).toList(),
...
class _QuestionCardState extends State<_QuestionCard> {
  ...
  void initState() {
    ...
    _timeCtrl = TextEditingController(text: q.timeLimitSeconds?.toString() ?? '');
  ...
  void _save() {
    widget.onUpdate(widget.question.copyWith(
      ...
      options: _optionCtrls.isNotEmpty ? _optionCtrls.map((c) => c.text.trim()).toList() : null,
      correctAnswer: _answerCtrl.text.trim(),
      explanation: _explCtrl.text.trim().isEmpty ? null : _explCtrl.text.trim(),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: bg, borderRadius: BorderRadius.circular(10), border: Border.all(color: border),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
              ...
              child: Row(children: [
                ...
                Expanded(
                  child: Text(
                    ...
                    style: GoogleFonts.inter(fontSize: 13, color: q.question.isEmpty ? mutedColor : textColor, height: 1.4),
                    maxLines: 2, overflow: TextOverflow.ellipsis,
                  ),
                ),
                const SizedBox(width: 6),
                Text(marksDisplay, style: GoogleFonts.inter(fontSize: 11, color: _coral, fontWeight: FontWeight.w600)),
                const SizedBox(width: 4),
                Icon(_expanded ? Icons.expand_less : Icons.expand_more, color: mutedColor, size: 20),
                IconButton(
            ...
            Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  ..._optionCtrls.asMap().entries.map((e) => Padding(
                    ...
                    child: Row(children: [
                      Text('${String.fromCharCode(65 + e.key)}. ',
                        style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: _coral)),
                      Expanded(
                        child: TextField(
                          controller: e.value,
                          style: GoogleFonts.inter(fontSize: 13, color: textColor),
                          decoration: _inputDec('Option ${String.fromCharCode(65 + e.key)}', isDark),
                          onChanged: (_) => _save(),
                        ),
                      ),
                    ]),
                ...
                TextField(
                  ...
                  style: GoogleFonts.inter(fontSize: 13, color: textColor),
                  decoration: _inputDec(
                    q.options != null ? 'A, B, C or D' : 'Enter correct answer',
                    isDark,
                  ),
                  onChanged: (_) => _save(),
                ),
                ...
                TextField(
                  ...
                  style: GoogleFonts.inter(fontSize: 13, color: textColor),
                  decoration: _inputDec('Why this is the correct answer…', isDark),
                  onChanged: (_) => _save(),
                ),
                const SizedBox(height: 12),
                ...
                Row(children: [
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    TextField(
                      ...
                      keyboardType: const TextInputType.numberWithOptions(decimal: true),
                      style: GoogleFonts.inter(fontSize: 13, color: textColor),
                      decoration: _inputDec('${widget.defaultMarks.toStringAsFixed(0)} (default)', isDark),
                      onChanged: (_) => _save(),
                    ),
                  ])),
                  const SizedBox(width: 10),
  ...
  InputDecoration _inputDec(String hint, bool isDark) {
    ...
    return InputDecoration(
      hintText: hint,
      hintStyle: GoogleFonts.inter(fontSize: 13, color: const Color(0xFF8E8B82)),
      filled: true, fillColor: fill,
...
class _ContextSheet extends StatefulWidget {
  ...
  final Function(String? sid, String? sname, List<String> cids, List<String> cnames) onSave;
...
class _ContextSheetState extends State<_ContextSheet> {
  ...
  Future<void> _load() async {
    ...
    if (widget.selectedSubjectId != null) {
      final s = subjects.where((x) => x.id == widget.selectedSubjectId).firstOrNull;
  ...
  Future<void> _selectSubject(Subject s) async {
    ...
    setState(() { _selectedSubject = s; _chapters = chapters; });
  }
        ...
        child: Column(
          children: [
            ...
            Container(width: 36, height: 4, decoration: BoxDecoration(
              color: mutedColor.withValues(alpha: 0.3), borderRadius: BorderRadius.circular(2),
            )),
            const SizedBox(height: 12),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: Row(children: [
                Text('Study Context', style: GoogleFonts.playfairDisplay(fontSize: 18, color: textColor)),
                const Spacer(),
                TextButton(
                  onPressed: () {
                    final names = _chapters
                        ...
                        .toList();
                    widget.onSave(
                      _selectedSubject?.id, _selectedSubject?.name,
                      _selectedChapterIds.toList(), names,
                    );
                  ...
                  SizedBox(
                    ...
                    child: ListView(
                      ...
                      children: [
                        ListTile(
                          dense: true,
                          title: Text('None', style: GoogleFonts.inter(fontSize: 13, color: mutedColor)),
                          selected: _selectedSubject == null,
                          selectedColor: _coral,
                          onTap: () => setState(() { _selectedSubject = null; _chapters = []; _selectedChapterIds = {}; }),
                        ),
                        ..._subjects.map((s) => ListTile(
                          dense: true,
                          title: Text(s.name, style: GoogleFonts.inter(fontSize: 13, color: textColor), overflow: TextOverflow.ellipsis),
                    ...
                    child: ListView(
                      ...
                      children: _selectedSubject == null
                          ? [Padding(padding: const EdgeInsets.all(16),
                              child: Text('Select a subject first', style: GoogleFonts.inter(fontSize: 13, color: mutedColor)))]
                          : _chapters.isEmpty
                          ? [Padding(padding: const EdgeInsets.all(16),
                              child: Text('No chapters', style: GoogleFonts.inter(fontSize: 13, color: mutedColor)))]
                          : [
                              CheckboxListTile(
                                dense: true,
                                title: Text('All chapters', style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: textColor)),
                                value: _selectedChapterIds.length == _chapters.length,
                                tristate: true,
                                activeColor: _coral,
                                onChanged: (_) {
                                  setState(() {
                                    if (_selectedChapterIds.length == _chapters.length) {
                                      ...
                                      _selectedChapterIds = _chapters.map((c) => c.id).toSet();
                                    }
                                  });
                                },
                              ),
                              ..._chapters.map((c) => CheckboxListTile(
                                dense: true,
                                title: Text(c.title, style: GoogleFonts.inter(fontSize: 13, color: textColor), overflow: TextOverflow.ellipsis),
