...
$currentExamJson''';
  ...
  String _typeInstruction(String type, int mcqOptionCount) => switch (type) {
    'mcq' => 'Generate multiple-choice questions with exactly $mcqOptionCount options each. The correctAnswer must EXACTLY match one of the options.',
    'true_false' => 'Generate true-or-false questions. options MUST be ["True", "False"] for every question. correctAnswer is "True" or "False".',
    'fill_blank' => 'Generate fill-in-the-blank questions. Use ___ in the question for each blank. Do not include options.',
    'short_answer' => 'Generate short-answer questions. No options. The correctAnswer is a 1-3 sentence response.',
    'long_answer' => 'Generate long-answer / essay questions. No options. The correctAnswer is a detailed multi-sentence response.',
    'mixed' => 'Generate a mix of question types (mcq, true_false, fill_blank, short_answer). Vary them throughout. Each question must have its "type" field set correctly.',
    _ => 'Generate questions appropriate for the topic.',
  };

  // ── Context builder ──────────────────────────────────────────────────────

  Future<String> _buildContextBlock(List<String> chapterIds, String query) async {
    if (chapterIds.isEmpty) return '';
    try {
      final retrieval = RetrievalService(_chunkRepo);
      final chunks = <String>[];
      for (final cid in chapterIds) {
        final found = await retrieval.searchChapter(cid, query.isEmpty ? 'overview' : query);
  ...
  ExamGenerationResult _parseResponse(String raw, String fallbackTitle, String fallbackType) {
    ...
    try {
      ...
      if (match == null) throw AppException.service('Could not parse exam response. Please try again.');
      try {
        json = jsonDecode(match.group(0)!) as Map<String, dynamic>;
      } catch (_) {
        throw AppException.service('Invalid exam format returned. Please try again.');
      }
    }
    ...
    if (rawQuestions.isEmpty) {
      throw AppException.service('No questions were generated. Try a different prompt.');
    }
  ...
  Future<void> _logUsage(Map<String, dynamic> responseData, String toolType) async {
    try {
