...
class StudyPlanAIService {
  ...
  Future<StudyPlanDraft> generatePlan({
    ...
    final system = '''You are an expert study planner. Create a day-by-day study plan.
...
Return ONLY valid JSON — no markdown, no explanation:
{
  ...
  "days": [
    ...
    final resp =
        await _post('https://api.openai.com/v1/chat/completions', apiKey, body);
  ...
  Future<StudyPlanDraft> refinePlan({
    ...
    final system = '''You are a study plan editor. Modify the plan based on the user's instruction.
...
${_draftToJson(current)}''';
  ...
  StudyPlanDraft _parse(String raw) {
    ...
    try {
      ...
      if (m == null) {
        throw AppException.service('Could not parse study plan. Please try again.');
      }
  ...
  Future<void> _logUsage(Map<String, dynamic> data, String tool) async {
    try {
      ...
      final inP = double.tryParse(
              await _settingsRepo.get(SettingKeys.inputTokenPrice) ?? '') ??
          _defaultInputPrice;
      final outP = double.tryParse(
  ...
  String _requireApiKey() {
    ...
    if (k.isEmpty) {
      throw AppException.configuration('Missing OpenAI API key.');
    }
    return k;
  }

  Future<http.Response> _post(String url, String key, String body) async {
    try {
      return await http
          .post(
            Uri.parse(url),
