...
async function setVectorColumn(table: string, id: string, column: string, values: number[]) {
  const vector = `[${values.map((value) => value.toFixed(8)).join(",")}]`;
  try {
    await prisma.$executeRawUnsafe(
      `UPDATE "${table}" SET "${column}" = $1::vector WHERE id = $2::uuid`,
      vector,
      id,
    );
  } catch {
...
async function main() {
  ...
  const textbook = await prisma.textbook.upsert({
    where: {
      subjectId_medium_classLevel_syllabus_partLabel: {
  ...
  const existingTextbookVersion = await prisma.textbookVersion.findFirst({
    where: {
  ...
  const textbookVersion = existingTextbookVersion
    ? await prisma.textbookVersion.update({
        where: { id: existingTextbookVersion.id },
        data: { isActive: true, status: "published" },
      })
    : await prisma.textbookVersion.create({
        data: {
          textbookId: textbook.id,
  ...
  const page = await prisma.page.upsert({
    ...
    create: {
      ...
      pageNumber: 1,
      rawText:
        "Photosynthesis is the process by which green plants prepare food using sunlight, carbon dioxide, and water. Chlorophyll helps trap sunlight. The process releases oxygen.",
      normalizedText:
        "photosynthesis is the process by which green plants prepare food using sunlight carbon dioxide and water chlorophyll helps trap sunlight the process releases oxygen",
      ocrUsed: false,
  ...
  console.log(
    `Seed complete. Demo accounts: admin@rightanswer.local, teacher@rightanswer.local, student@rightanswer.local with password Password123!`,
  );
}
